...
class KnowledgeGraph(object):
    ...
    def _load_reviews(
        self, dataset, word_tfidf_threshold=0.1, word_freq_threshold=5000
    ):
        print("Load reviews...")
    ...
    def heuristic_search(self, uid, pid, pattern_id, trim_edges=False):
        if trim_edges and self.top_matches is None:
            raise Exception(
                "To enable edge-trimming, must set top_matches of users first!"
            )
        if trim_edges:
        ...
        if pattern_id == 1:  # OK
            ...
            for pid_u in pids_u:
                ...
                et_ids = set(
                    _get(PRODUCT, pid_u, relation)
                )  # USER->PURCHASE->PRODUCT->relation->node2
            ...
            for uid_p in uids_p:
                wids_u_p = set(
                    _get(USER, uid_p, MENTION)
                )  # PRODUCT->PURCHASE->USER->MENTION->WORD
