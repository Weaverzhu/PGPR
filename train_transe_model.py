...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while dataloader.has_next():
            # Set learning rate.
            lr = args.lr * max(
                1e-4, 1.0 - dataloader.finished_word_num / float(words_to_train)
            )
            ...
            if steps % args.steps_per_checkpoint == 0:
                logger.info(
                    ...
                    + "Words: {:d}/{:d} | ".format(
                        dataloader.finished_word_num, words_to_train
                    )
                    + "Lr: {:.5f} | ".format(lr)
...
def extract_embeddings(args):
    ...
    embeds = {
        USER: state_dict["user.weight"]
        .cpu()
        .data.numpy()[:-1],  # Must remove last dummy 'user' with 0 embed.
...
def main():
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0, help="weight decay for adam."
    )
    parser.add_argument("--l2_lambda", type=float, default=0, help="l2 lambda")
    parser.add_argument(
        "--max_grad_norm", type=float, default=5.0, help="Clipping gradient."
    )
    parser.add_argument(
        "--embed_size", type=int, default=100, help="knowledge embedding size."
    )
    parser.add_argument(
        "--num_neg_samples", type=int, default=5, help="number of negative samples."
    )
    parser.add_argument(
