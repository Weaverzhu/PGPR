...
class KnowledgeEmbedding(nn.Module):
    def __init__(self, dataset, args):
        ...
        self.relations = edict(
            ...
            mentions=edict(
                et="word", et_distrib=self._make_distrib(dataset.review.word_distrib)
            ),
            describe_as=edict(
                et="word", et_distrib=self._make_distrib(dataset.review.word_distrib)
            ),
    ...
    def _entity_embedding(self, vocab_size):
        ...
        weight = torch.FloatTensor(vocab_size + 1, self.embed_size).uniform_(
            -initrange, initrange
        )
    ...
    def compute_loss(self, batch_idxs):
        ...
        up_loss, up_embeds = self.neg_loss(
            "user", "purchase", "product", user_idxs, product_idxs
        )
        regularizations.extend(up_embeds)
        loss = up_loss

        # user + mentions -> word
        uw_loss, uw_embeds = self.neg_loss(
            "user", "mentions", "word", user_idxs, word_idxs
        )
        regularizations.extend(uw_embeds)
        loss += uw_loss

        # product + describe_as -> word
        pw_loss, pw_embeds = self.neg_loss(
            "product", "describe_as", "word", product_idxs, word_idxs
        )
        regularizations.extend(pw_embeds)
        loss += pw_loss

        # product + produced_by -> brand
        pb_loss, pb_embeds = self.neg_loss(
            "product", "produced_by", "brand", product_idxs, brand_idxs
        )
        ...
        pc_loss, pc_embeds = self.neg_loss(
            "product", "belongs_to", "category", product_idxs, category_idxs
        )
        ...
        pr1_loss, pr1_embeds = self.neg_loss(
            "product", "also_bought", "related_product", product_idxs, rproduct1_idxs
        )
        ...
        pr2_loss, pr2_embeds = self.neg_loss(
            "product", "also_viewed", "related_product", product_idxs, rproduct2_idxs
        )
        ...
        pr3_loss, pr3_embeds = self.neg_loss(
            "product",
            "bought_together",
            "related_product",
            product_idxs,
    ...
    def neg_loss(
        self, entity_head, relation, entity_tail, entity_head_idxs, entity_tail_idxs
    ):
...
def kg_neg_loss(
    ...
    pos_logits = (
        torch.bmm(pos_vec, example_vec).squeeze() + relation_bias
    )  # [batch_size]
